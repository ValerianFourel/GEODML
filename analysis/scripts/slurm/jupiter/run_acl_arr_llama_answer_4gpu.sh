...
stop_submit_epoch=$((step_start_epoch + 3000))
...
: "${ACL_ARR_RERANK_RECOVERY_RESULTS:?set the completed rerank recovery results}"
: "${ACL_ARR_VENV:?set the existing vLLM virtual environment}"
: "${ACL_ARR_ANSWER_ROOT:?set a fresh answer/log directory}"
: "${ACL_ARR_ANSWER_JOB_ID:?set the existing approved allocation ID}"
: "${ACL_ARR_ANSWER_APPROVED_WALLTIME:?set the approved answer step limit}"
: "${ACL_ARR_ANSWER_ESTIMATE:?set the approved answer estimate}"
: "${GEODML_ANSWER_COMMIT:?set the answer checkout commit}"

if [[ "$ACL_ARR_ANSWER_APPROVED_WALLTIME" != "01:00:00" ]]; then
    echo "ERROR: this answer step was approved only for 01:00:00" >&2
    exit 2
fi
if [[ "${SLURM_JOB_ID:-}" != "$ACL_ARR_ANSWER_JOB_ID" ||
...
cd "$REPOSITORY_ROOT"
test "$(git rev-parse HEAD)" = "$GEODML_ANSWER_COMMIT"
git diff --quiet HEAD --
test -x "$ACL_ARR_VENV/bin/python"
test -x "$ACL_ARR_VENV/bin/vllm"
test -d "$ACL_ARR_ANSWER_ROOT"
test ! -e "$ACL_ARR_ANSWER_ROOT/results"
...
SERVER_LOG="$ACL_ARR_ANSWER_ROOT/vllm-answer.log"
answer_args=(
    --run-root "$ACL_ARR_RUN_ROOT"
    --output-dir "$ACL_ARR_ANSWER_ROOT/results"
    --base-url "$SERVER_URL"
    --rerank-recovery-results "$ACL_ARR_RERANK_RECOVERY_RESULTS"
    --stop-submit-epoch "$stop_submit_epoch"
    --approved-walltime "$ACL_ARR_ANSWER_APPROVED_WALLTIME"
    --allocation-estimate "$ACL_ARR_ANSWER_ESTIMATE"
)
if [[ -n "${ACL_ARR_ANSWER_RESUME_FROM:-}" ]]; then
    answer_args+=(--resume-from "$ACL_ARR_ANSWER_RESUME_FROM")
fi

# Validate frozen answer inputs and the completed rerank recovery before weights.
python3 analysis/scripts/run_acl_arr_pilot_answers.py \
...
server_args=(
    ...
    --max-model-len 41984 --gpu-memory-utilization 0.90
...
print("ANSWER_VLLM_CLI=PASS; no model engine started")
PY
...
print("ANSWER_GRAMMAR=PASS; original answer schema; no model engine loaded")
PY

python3 - "$SERVER_PORT" "$ACL_ARR_RUN_ROOT" "$ACL_ARR_ANSWER_ROOT" <<'PY'
import socket
import sys
...
print("ANSWER_PORT=AVAILABLE")
PY

# Atomic one-use guard keeps logs and incomplete attempts intact on failure.
mkdir "$ACL_ARR_ANSWER_ROOT/worker-started"
python3 -m pip freeze > "$ACL_ARR_ANSWER_ROOT/pip-freeze.txt"
nvidia-smi --query-gpu=index,name,memory.total,memory.used --format=csv \
    > "$ACL_ARR_ANSWER_ROOT/gpu-before.csv"
printf 'ANSWER_JOB_ID=%s\nANSWER_COMMIT=%s\nANSWER_ROOT=%s\n' \
    "$SLURM_JOB_ID" "$GEODML_ANSWER_COMMIT" "$ACL_ARR_ANSWER_ROOT"
...
echo "START_ANSWER_SERVER model=$MODEL_ID revision=$MODEL_REVISION context=41984"
...
echo "ANSWER_SERVER_READY model=$MODEL_ID"
python3 analysis/scripts/run_acl_arr_pilot_answers.py "${answer_args[@]}"
echo "ANSWER_WORKER=PASS; existing allocation remains owned by its original shell"
