...
def prepare(selection_manifest: Path, output: Path, *, axis_bins: int = 20) -> dict[str, Any]:
    if output.exists():
    ...
    shutil.copyfile(verified["prompts"], prompt_target)
    ...
    } for row in sorted(population, key=lambda item: item.candidate_id)))
    ...
    result = {
        ...
        "files": {
            ...
            for path in (prompt_target, records_target, priority_target)
        },
    }
