...
against the positive `SLURM_JOB_CPUS_PER_NODE` value. Batch steps that do not
provide that job-level variable fall back to `SLURM_CPUS_ON_NODE`; ordinary
`srun` steps do not, because that value describes only the step. If `Exclusive`
is present it remains authoritative, so conflicting scheduler evidence is
rejected. Missing or conflicting scheduler evidence stops the job. This path
retains literal loopback binding, per-run native authentication, and
loopback-only NCCL/Gloo transport. It does not claim that a private kernel
