...
def run_full_model(X, Y, D, t_name, path, y_name):
    ...
    # ── 3. Feature importances (refit LGBM on full data) ─────────────────
...
def plot_feature_importances(all_results):
    """Heatmap of LGBM feature importances for ml_l (outcome model), rank_delta."""
    treatments = []
    imp_matrix = []
    for t_name in ["T1", "T2", "T3", "T4"]:
        for path in ["code", "llm"]:
    ...
    ax.set_title("LGBM Feature Importances — Outcome Model ml_l (Y=rank_delta ~ X)\n"
