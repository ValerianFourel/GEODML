#!/usr/bin/env python3
"""
GEODML — Full DML Experiment Suite (32 models)

Factorial design:
  2 outcomes      × 4 treatments × 2 measurement paths × 2 DML methods = 32
...
Output:
  test/results/all_experiments.csv      — 32-row summary table
  test/results/all_experiments.json     — full results + metadata
  test/results/heatmap_pvalues.png      — p-value heatmap (treatment × config)
  test/results/coef_grid.png            — coefficient grid plot
"""
...
# Treatments that need binarization for IRM (T2, T3 are already binary)
...
# ── Model fitting ────────────────────────────────────────────────────────────
def fit_plr(X, Y, D):
    """Fit DoubleML PLR with LGBM regressors."""
...
def fit_irm(X, Y, D):
    """Fit DoubleML IRM with LGBM regressor (outcome) + classifier (propensity)."""
    data = dml.DoubleMLData.from_arrays(x=X, y=Y, d=D)
...
def extract_results(model):
    """Pull coefficient, SE, CI, p-value from a fitted DoubleML model."""
...
def run_all():
    df = load_data()
    print(f"Running 32 DML experiments (2 Y × 4 T × 2 paths × 2 methods)\n")
    ...
    if len(sig) > 0:
        ...
        for _, r in sig.iterrows():
            ...
            print(f"  {r['outcome']:<10} ~ {r['treatment']}_{r['path']:<4} ({r['method']}): "
                  f"θ̂={r['coef']:+.3f}, p={r['p_val']:.4f}{r['significance']}  → {direction}")
...
def save_outputs(results_df):
    ...
    output = {
        "study": "GEODML DML Experiment Suite",
        "design": "2 outcomes × 4 treatments × 2 paths × 2 methods = 32 experiments",
        "outcomes": {"pre_rank": "SERP position (1-19, lower=better)",
                     "post_rank": "LLM re-ranked position (1-10, lower=better)"},
...
def plot_heatmap(results_df):
    """P-value heatmap: rows = treatment×path, columns = Y×method."""
    valid = results_df[results_df["coef"].notna()].copy()
    ...
    # Annotate cells with coef and p-value
    ...
    ax.set_title("DML Experiment Suite: P-values and Coefficients\n"
...
def plot_coef_grid(results_df):
    """2×2 grid of coefficient plots: rows=Y, cols=method."""
    valid = results_df[results_df["coef"].notna()].copy()
    ...
    for idx, (y_name, method) in enumerate(titles):
        ...
        ax.set_xlabel("Causal effect (θ̂)", fontsize=9)
        ax.invert_yaxis()

    fig.suptitle("DML Coefficient Estimates (blue=p<0.05, red=p<0.1, grey=n.s.)",
                 fontsize=13, y=1.01)
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("GEODML — DML Experiment Suite (32 models)")
