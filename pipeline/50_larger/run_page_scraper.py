#!/usr/bin/env python3
"""Scrape page features for DML causal inference experiment — 50_larger variant.

Reads a results CSV from 50_larger/results/, fetches each unique URL once,
extracts treatments (T1-T4) and confounders (X1-X10) via code + LLM + APIs.

Usage:
  python 50_larger/run_page_scraper.py                          # code extraction only
  python 50_larger/run_page_scraper.py --llm                    # + LLM treatment analysis
  python 50_larger/run_page_scraper.py --pagerank               # + X1 domain authority
  python 50_larger/run_page_scraper.py --whois                  # + X2 domain age
  python 50_larger/run_page_scraper.py --lcp                    # + X4 page speed
  python 50_larger/run_page_scraper.py --all                    # all phases
  python 50_larger/run_page_scraper.py --max-urls 5             # test with 5 URLs
"""
...
def _timeout_handler(signum, frame):
    raise FetchTimeout("hard timeout")

# Add project root to path
PROJECT_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(PROJECT_ROOT))

from src.config import GOOGLE_API_KEY, HF_TOKEN, OPENPAGERANK_KEY
...
def _fetch_page_inner(url: str, session: requests.Session) -> tuple[str | None, int | None, str | None]:
    try:
        ...
        # Detect encoding from headers, fall back to utf-8
        ...
        return None, None, "chunked_encoding_error"
...
def fetch_page(url: str, session: requests.Session) -> tuple[str | None, int | None, str | None]:
    """Fetch page with a hard signal-based timeout of 45s to prevent hangs."""
    old_handler = signal.signal(signal.SIGALRM, _timeout_handler)
    signal.alarm(45)  # hard kill after 45 seconds
    try:
        return _fetch_page_inner(url, session)
    except FetchTimeout:
...
def run_phase1(url_list: list[dict], existing: dict[str, dict],
               ...
               progress_path: Path = None) -> dict[str, dict]:
    ...
    for i, entry in enumerate(remaining, 1):
        ...
        # Try HTML cache first to avoid re-fetching
        cached_html = _load_html_cache(url)
        if cached_html:
            html, status, fetch_err = cached_html, 200, None
        ...
        if i % 10 == 0:
            if progress_path:
                save_progress(progress_path, existing)
            print(f"  [progress saved: {len(existing)} URLs]")

        if not cached_html:  # Only rate-limit for network fetches
...
def main():
    parser = argparse.ArgumentParser(description="Scrape page features for DML (50_larger)")
    parser.add_argument("--input", type=str, default=None,
                        help="Input CSV (default: 50_larger/results/all_results_searxng.csv)")
    ...
    if args.input:
        ...
        input_path = RESULTS_DIR / "all_results_searxng.csv"
    ...
    # Save progress to data/ dir (results/ root files keep getting deleted)
    data_dir = Path(__file__).resolve().parent / "data"
    data_dir.mkdir(exist_ok=True)
    prog_path = data_dir / f"page_features_progress_{tag}.json"
