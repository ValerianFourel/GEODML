#!/usr/bin/env python3
"""
GEODML — Full Model Diagnostics (RF nuisance learners) — 50_larger variant

Same as test_full but uses RandomForestRegressor instead of LGBM for nuisance models.
Reports RF Gini importances instead of LGBM split counts.
...
def print_tables(results_df, all_results):
    for y_name in OUTCOMES:
        ...
        print(f"\n{'=' * 95}")
        print(f"Y = {y_name}  (RF nuisance learners)")
        print(f"{'=' * 95}")
    ...
    if available_keys:
        ...
        print(f"\n{'=' * 95}")
        print("RF FEATURE IMPORTANCES — ml_l (Y ~ X) — rank_delta models (% Gini importance)")
        print("=" * 95)
...
def plot_dml_vs_ols(results_df):
    ...
    fig.suptitle("DML(RF) vs OLS (50_larger)\n(blue=p<0.05, red=p<0.1, grey=n.s.)",
                 fontsize=12, y=1.03)
    plt.tight_layout()
...
def plot_nuisance_r2(results_df):
    ...
    fig.suptitle("RF Nuisance Model Performance (50_larger)\n"
...
def plot_feature_importances(all_results):
    ...
    ax.set_title("RF Feature Importances (50_larger) — ml_l (Y=rank_delta ~ X)\n"
                 "% Gini importance per confounder", fontsize=11)
    plt.colorbar(im, ax=ax, label="% of importance", shrink=0.8)
    plt.tight_layout()
...
def plot_ols_confounder_coefs(all_results):
    ...
    fig.suptitle("OLS Confounder Coefficients (50_larger RF, Y = rank_delta)\n"
