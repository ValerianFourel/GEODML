#!/usr/bin/env python3
"""
GEODML — Full Model Diagnostics

For each DML experiment, extract and report:
  1. Causal estimate (θ̂, SE, p-value, CI)
  2. Nuisance model performance (cross-validated R² for ml_l and ml_m)
  3. Confounder feature importances (RF Gini importance, both nuisance models)
  4. Naive OLS comparison (treatment + confounders, no DML orthogonalization)
  5. Descriptive statistics for all variables

Runs on: Y = rank_delta, all 4 treatments × 2 paths, PLR only (8 core models).
"""
...
def prepare(df, y_col, d_col):
    """Return clean X, Y, D arrays and the working subset."""
    cols = CONFOUNDERS + [d_col, y_col]
    ...
    X_imp = imputer.fit_transform(sub[CONFOUNDERS])
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X_imp)

    return X_scaled, Y, D, sub, scaler


# ── Run one full model ───────────────────────────────────────────────────────
def run_full_model(X, Y, D, t_name, path, y_name):
    """Run DML PLR + extract all diagnostics."""
    result = {}
    ...
    # ── 2. Nuisance model R² (from cross-validated predictions) ──────────
    ...
    # ── 3. Feature importances (refit RF on full data) ─────────────────
    ml_l_full = RandomForestRegressor(n_estimators=500, max_depth=5,
                                 random_state=42, n_jobs=-1)
    ml_l_full.fit(X, Y)
    ml_m_full = RandomForestRegressor(n_estimators=500, max_depth=5,
                                 random_state=42, n_jobs=-1)
    ml_m_full.fit(X, D)
    ...
    imp_l_pct = imp_l / imp_l.sum() * 100
    imp_m_pct = imp_m / imp_m.sum() * 100

    result["importances"] = {
        "ml_l": {c: {"gini": round(float(imp_l[i]), 4), "pct": round(float(imp_l_pct[i]), 1)}
                 for i, c in enumerate(CONFOUNDERS)},
        "ml_m": {c: {"gini": round(float(imp_m[i]), 4), "pct": round(float(imp_m_pct[i]), 1)}
                 for i, c in enumerate(CONFOUNDERS)},
    }

    # ── 4. Naive OLS (no DML orthogonalization) ─────────────────────────
    XD = np.column_stack([D, X])
    ols = LinearRegression().fit(XD, Y)
    ols_coef_d = ols.coef_[0]
    # Bootstrap SE for OLS treatment coef
    n_boot = 1000
    ...
    result["ols"] = {
        ...
        "p_val": float(ols_p),
        "r2": float(ols_r2),
        "coef_confounders": {c: float(ols.coef_[i + 1]) for i, c in enumerate(CONFOUNDERS)},
    ...
    for y_name, y_col in OUTCOMES.items():
        ...
        for t_name in ["T1", "T2", "T3", "T4"]:
            for path in ["code", "llm"]:
                ...
                X, Y, D, sub, scaler = prepare(working_df, y_col, d_col)
                n = len(Y)

                result = run_full_model(X, Y, D, t_name, path, y_name)
                result["meta"] = {
                ...
                print(f"{label}  n={n:<4}  "
                      f"DML: θ̂={d['coef']:+.3f} p={d['p_val']:.4f}{stars:<3}  "
                      f"OLS: β={o['coef_treatment']:+.3f} p={o['p_val']:.4f}{ols_stars:<3}  "
                      f"R²(Y~X)={nu['r2_ml_l']:+.3f}  R²(D~X)={nu['r2_ml_m']:+.3f}")
...
def print_tables(results_df, all_results):
    ...
    # ── Table 2: OLS confounder coefficients (rank_delta models) ─────────
    print(f"\n{'=' * 95}")
    ...
    for c in CONFOUNDERS:
        row = f"{CONFOUNDER_LABELS[c]:<25}"
        for t_name in ["T1", "T2", "T3", "T4"]:
            for path in ["code", "llm"]:
                key = f"rank_delta__{t_name}_{path}"
                coef = all_results[key]["ols"]["coef_confounders"][c]
                row += f"  {coef:+7.3f}"
        print(row)

    # ── Table 3: Feature importances (rank_delta models) ─────────────────
    print(f"\n{'=' * 95}")
    print("RF FEATURE IMPORTANCES — ml_l (Y ~ X) — rank_delta models (% of total splits)")
    print("=" * 95)
    ...
    for c in CONFOUNDERS:
        ...
        for t_name in ["T1", "T2", "T3", "T4"]:
            for path in ["code", "llm"]:
                key = f"rank_delta__{t_name}_{path}"
                pct = all_results[key]["importances"]["ml_l"][c]["pct"]
                row += f"  {pct:6.1f}%"
        print(row)


def plot_dml_vs_ols(results_df):
    """Scatter: DML θ̂ vs OLS β for each outcome."""
    ...
    for idx, y_name in enumerate(OUTCOMES):
        ...
        ax.set_xlabel("OLS β (naive)", fontsize=10)
        ax.set_ylabel("DML θ̂ (causal)", fontsize=10)
        ax.set_title(f"Y = {y_name}", fontsize=11)

    fig.suptitle("DML vs OLS: Treatment Effect Estimates\n"
                 "(blue=p<0.05, red=p<0.1, grey=n.s.; dashed=identity)",
                 fontsize=12, y=1.03)
    plt.tight_layout()
...
def plot_nuisance_r2(results_df):
    """Bar chart of nuisance model R² for each experiment."""
    fig, axes = plt.subplots(1, 3, figsize=(16, 5))
    for idx, y_name in enumerate(OUTCOMES):
        ax = axes[idx]
    ...
    fig.suptitle("Nuisance Model Performance (Cross-Validated R²)\n"
                 "Negative R² = confounders predict worse than the mean",
                 fontsize=12, y=1.03)
    plt.tight_layout()
...
def plot_feature_importances(all_results):
    """Heatmap of RF feature importances for ml_l (outcome model), rank_delta."""
    treatments = []
    imp_matrix = []
    for t_name in ["T1", "T2", "T3", "T4"]:
        for path in ["code", "llm"]:
            ...
            row = [all_results[key]["importances"]["ml_l"][c]["pct"] for c in CONFOUNDERS]
    ...
    ax.set_xticks(range(len(CONFOUNDERS)))
    ax.set_xticklabels([CONFOUNDER_LABELS[c] for c in CONFOUNDERS], fontsize=8, rotation=35, ha="right")
    ...
    ax.set_title("RF Feature Importances — Outcome Model ml_l (Y=rank_delta ~ X)\n"
...
def plot_ols_confounder_coefs(all_results):
    """Bar chart of OLS confounder coefficients for rank_delta models."""
    fig, axes = plt.subplots(2, 4, figsize=(18, 8), sharey=True)

    idx = 0
    for t_name in ["T1", "T2", "T3", "T4"]:
        for pi, path in enumerate(["code", "llm"]):
            ax = axes[pi, ["T1", "T2", "T3", "T4"].index(t_name)]
            key = f"rank_delta__{t_name}_{path}"
            coefs = all_results[key]["ols"]["coef_confounders"]
            d_coef = all_results[key]["ols"]["coef_treatment"]
            r2 = all_results[key]["ols"]["r2"]

            labels = [CONFOUNDER_LABELS[c].replace(" ", "\n", 1) for c in CONFOUNDERS]
            vals = [coefs[c] for c in CONFOUNDERS]
            ...
            ax.set_title(f"{t_name} {TREATMENT_LABELS[t_name]}\n({path}) R²={r2:.3f}",
                         fontsize=9)
            ax.set_xlabel("OLS β (standardized)", fontsize=7)
            ax.invert_yaxis()

    fig.suptitle("OLS Confounder Coefficients (Y = rank_delta, standardized confounders)\n"
                 "Blue = positive (promotes), Red = negative (demotes)",
                 fontsize=12, y=1.02)
    plt.tight_layout()
...
def save_descriptive_stats(df):
    """Save descriptive statistics for all variables."""
    ...
    print(f"\n{'=' * 95}")
    print("DESCRIPTIVE STATISTICS (n=399, rows with valid rank_delta)")
    print("=" * 95)
