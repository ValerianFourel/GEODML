#!/usr/bin/env python3
"""Extract domain, url, keyword, engine, source from 50_larger results JSON files into a single CSV."""
...
def extract_rows(json_path: Path) -> list[dict]:
    ...
    engine = data.get("serp_engine", "")
    model = data.get("chat_model", "none")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Extract results from 50_larger JSON files into CSV")
    ...
    if not results_dir.is_dir():
        print("Error: 50_larger/results/ directory not found", file=sys.stderr)
        sys.exit(1)
    ...
    if not json_files:
        print("No matching JSON files found in 50_larger/results/", file=sys.stderr)
        sys.exit(1)
